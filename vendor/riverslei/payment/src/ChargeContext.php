...
class ChargeContext
{
    ...
    public function initCharge($channel, array $config)
    {
        ...
        try {
            switch ($channel) {
                case Configv::ALI_CHANNEL_WAP:
                    $this->channel = new AliWapCharge($config);
                    break;
                case Configv::ALI_CHANNEL_APP:
                    $this->channel = new AliAppCharge($config);
                    break;
                case Configv::ALI_CHANNEL_WEB:
                    $this->channel = new AliWebCharge($config);
                    break;
                case Configv::ALI_CHANNEL_QR:
                    $this->channel = new AliQrCharge($config);
                    break;
                case Configv::ALI_CHANNEL_BAR:
                    $this->channel = new AliBarCharge($config);
                    break;

                case Configv::WX_CHANNEL_APP:
                    $this->channel = new WxAppCharge($config);
                    break;
                case Configv::WX_CHANNEL_LITE:// 小程序支付与公众号支付一样，仅仅是客户端的调用方式不同
                case Configv::WX_CHANNEL_PUB:
                    $this->channel = new WxPubCharge($config);
                    break;
                case Configv::WX_CHANNEL_WAP:
                    $this->channel = new WxWapCharge($config);
                    break;
                case Configv::WX_CHANNEL_QR:
                    $this->channel = new WxQrCharge($config);
                    break;
                case Configv::WX_CHANNEL_BAR:
                    $this->channel = new WxBarCharge($config);
                    break;

                case Configv::CMB_CHANNEL_WAP:
                case Configv::CMB_CHANNEL_APP:
